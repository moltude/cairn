...
from cairn.core.config import load_config, normalize_onx_icon_name
...
def show() -> None:
    ...
    console.print(
        f"  Unmapped detection: [cyan]{'Enabled' if summary['unmapped_detection_enabled'] else 'Disabled'}[/]"
    )
    console.print(
        f"  Icon name prefixes: [cyan]{'Enabled' if summary.get('use_icon_name_prefix') else 'Disabled'}[/]"
    )
    console.print(f"  Default icon: [cyan]{summary.get('default_icon', 'Location')}[/]")
    console.print(
        f"  Default color: [cyan]{summary.get('default_color', 'rgba(8,122,255,1)')}[/]"
    )
...
def export() -> None:
    ...
    console.print(
        f"[bold green]✔[/] Configuration template exported to [underline]{output_path}[/]"
    )
...
def validate(config_file: Path = typer.Argument(..., help="Config file to validate")):
    ...
    try:
        ...
        console.print(
            f"[bold green]✔[/] Configuration file is valid: [underline]{config_file}[/]"
        )
        summary = cfg.get_config_summary()
...
def set_default_icon(icon: str = typer.Argument(..., help="Icon name")):
    ...
    if icon_canon is None:
        ...
        console.print(
            "[dim]Tip: run 'cairn config show' to see icons already used in your mappings[/]"
        )
        raise typer.Exit(1)
    ...
    console.print(
        f"[bold green]✔[/] Default icon set to: [cyan]{icon_canon}[/] (saved to {config_path})"
    )
...
def set_default_color(color: str = typer.Argument(..., help="Color in rgba format")):
    ...
    if not (
        color.startswith("#")
        or color.lower().startswith(("rgb", "rgba"))
        or re.fullmatch(r"[0-9a-fA-F]{6}", color or "")
    ):
        console.print("[bold red]❌ Error:[/] Invalid color format")
    ...
    console.print(
        f"[bold green]✔[/] Default color set to: [cyan]{onx_color}[/] (saved to {config_path})"
    )
